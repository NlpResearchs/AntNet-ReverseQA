...
from utils import read_file, make_instances, \
    make_corpus, generate_vocabulary, \
    split_train_dev, drop_instances_to_excel, \
    gather_question_dict, split_template, gather_samples, \
    gather_targets_for_samples, read_file_list
...
if __name__ == "__main__":
    ...
    add_data = config["train_config"]["ADD_DATA"]
    ...
    drop_path = config["train_config"]["DROP_JUDGE_DEV_PATH"]
    ...
    samples_all = read_file_list([add_data, data_path])
    samples_pretrained, _, _ = read_file(add_data)
    ans_max_len = config["train_config"]["ANS_MAX_LEN"]
    que_max_len = config["train_config"]["QUE_MAX_LEN"]
    char_corpus, word_corpus = make_corpus(samples_all)
    # default sen2id from char_corpus
    sen2id = generate_vocabulary(sen2id_path, char_corpus)
    question2targets = gather_targets_for_samples(samples_pretrained, True,
    ...
    question2index = gather_question_dict(samples_pretrained)
    question2index, question2template = split_template(samples_pretrained, question2index)
    ...
    #                               is_training=True,
    #                               need_augment=True)
    instances = make_instances(samples_pretrained,
                               w2v_encoder.char_voc,
    ...
    instances_train, instances_valid = split_train_dev(instances,
                                                       dev_size=0.1)
