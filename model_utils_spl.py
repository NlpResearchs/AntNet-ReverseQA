...
def compute_cross_entropy_everysample(logits, truth):
    # logits: [batch_size, nb_classes]
    logits = tf.to_float(logits)
    return tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=truth)
...
def evaluation(sess, valid_graph, dev_data_stream, epoch,out_path=None):
    ...
    for batch_index in range(dev_data_stream.get_nb_batch()):  # for each batch
        ...
        feed_dict = valid_graph.create_feed_dict(cur_batch, np.array([epoch]))
...
def preserve_results(x1,x2,x3,pereservePath):
    result_preserve=pd.DataFrame(x1,columns=['questions'])
    result_preserve['answers'] = x2
    result_preserve['w_value'] = x3
    result_preserve.to_excel(pereservePath) 
    ...
    for epoch in range(nb_epoch):
        ...
        for batch_index in tqdm(range(data_stream_train.get_nb_batch())):
            ...
            if answer_understander_train.use_bert:
                cur_batch.update_bert_vector()
            feed_dict = answer_understander_train.create_feed_dict(cur_batch,np.array([epoch]))
            
            loss_everysample, tau, w, new_loss_everysample, loss, labels, logits = sess.run([
                answer_understander_train.loss_everysample,
                answer_understander_train.tau,
                answer_understander_train.w,
                answer_understander_train.new_loss_everysample,
                answer_understander_train.loss,
                answer_understander_train.labels,
                answer_understander_train.logits],
                feed_dict=feed_dict)            
            w_lst.extend(list(w))
            q_lst.extend(cur_batch.questions)
            a_lst.extend(cur_batch.answers)
            loss, _, = sess.run([
                answer_understander_train.loss,
        ...
        acc, _, _ = evaluation(sess, answer_understander_valid, data_stream_valid,epoch)
