...
class AnswerUnderstander(object):
    ...
    def create_feed_dict(self, cur_batch, epoch):
    ...
    def create_model_graph(self):
        ...
        logits = model_utils.full_connect_layer(end_repr,
                                                ...
                                                is_training=self.is_training)
        self.logits = logits
        self.loss_everysample = model_utils.compute_cross_entropy_everysample(logits, self.labels)
        ...
        k_0 = tf.constant(10,dtype = tf.int32) 
        tau_0 = tf.constant(1.5, dtype = tf.float64) 
        tau = tf.cond(k_0<self.epoch[0] , lambda:((self.epoch - k_0)/k_0)*tau_0, lambda:((self.epoch + k_0)/k_0)*tau_0)
        self.tau = tau
        tau_new = tf.cast(self.tau, dtype = tf.float32)
        self.w = tf.cond(k_0<self.epoch[0] , lambda:1/(1+tf.exp(5*(-self.loss_everysample+tau_new))), lambda:1/(1+tf.exp(5*(self.loss_everysample-tau_new))))
        self.new_loss_everysample = tf.multiply(self.w,self.loss_everysample)
