...
from model import AnswerUnderstander
from bert_encoder import BertEncoder
from sklearn import metrics
import matplotlib.pyplot as plt
# from lstm_encoder import LSTMENCODER
from utils import read_file, make_instances, gather_targets_for_samples, \
...
def test():
    ...
    instances_judge_dev = make_instances(
        judge_samples,
        char_voc,
        word_voc,
    ...
    with tf.Graph().as_default():
        ...
        judge_acc = evaluation(sess, answer_understander_dev,
                               data_stream_judge_dev, 'result_{}.txt'.format(loop_index))
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
        ...
        accuracy, predict_result, true_result = evaluation(sess, answer_understander_dev,
                                                           data_stream_judge_dev, 'result_{}.txt'.format(1))
        target_names = ['True', 'False', 'Uncertain']
        print(metrics.f1_score(true_result, predict_result, average='macro'))

        # get the template representation and corresponding label
        # template_repr = []
        # template_label = []
        # for batch_index in range(data_stream_judge_template.get_nb_batch()):  # for each batch
        #     cur_batch = data_stream_judge_template.get_batch(batch_index)
        ...
        # template_answer = np.array([instance['answer'] for instance in instances_judge_template])
        #
        # dev_repr = []
        # for batch_index in range(data_stream_judge_dev.get_nb_batch()):  # for each batch
        #     cur_batch = data_stream_judge_dev.get_batch(batch_index)
        #     feed_dict = answer_understander_dev.create_feed_dict(cur_batch)
        ...
        # max_cosine = []
        # match_answers = []
        # for instance_index in range(len(instances_judge_dev)):
        #     instance = instances_judge_dev[instance_index]
        #     assert (np.array(question2label[instance['question']]) == template_label \
        #         [question2id[instance['question']][0]:question2id[instance['question']][-1] + 1]).all
        #     instance_template_repr = template_repr[
        #                              question2id[instance['question']][0]:question2id[instance['question']][-1] + 1, :]
        ...
        #     match_labels.append(np.array(question2label[instance['question']])[np.argmax(cos_distance)])
        #     match_answers.append(template_answer[np.array(question2id[instance['question']])[np.argmax(cos_distance)]])
        ...
        # # fuse the result
        # fused_result = [match_labels[i] if float(max_cosine[i]) > 0.9 else predict_result[i] for i in
